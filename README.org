...
- On Wayland, there are currently several issues, including broken animated backgrounds on NVIDIA and incorrect UI element sizing on all GPUs.
  - These are caused by a bug in the current GTK package in Guix.
  - The issues do not exist in =gnome-team= branch and will be resolved once the changes are merged.
  - If you need animated backgrounds or correct UI sizing right now, you can fall back to X11 by launching the app with =GDK_BACKEND=x11=.
