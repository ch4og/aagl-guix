...
** Configure telemetry hosts blocking
...
- Module ~(aagl)~ provides all packages *except nvidia variants*.
...
- Module ~(aagl packages nvidia)~ provides nvidia variants:
  - ~the-honkers-railway-launcher-nvidia~
  - ~an-anime-game-launcher-nvidia~
  - ~sleepy-launcher-nvidia~
  - ~honkers-launcher-nvidia~

  You can select a specific nvidia driver version (like in nonguix):
  - In code: use variables such as ~an-anime-game-launcher-nvidia-580~,
    ~an-anime-game-launcher-nvidia-595~, or
    ~an-anime-game-launcher-nvidia-new-feature~.
  - The unversioned ~an-anime-game-launcher-nvidia~ uses the default driver.
